...
void ws281x_256square(uint8_t *pic_index_arr)
{
	int16_t i = 0;
	int16_t j = 0;
    for(i = 0; i < PIXEL_NUM; ++i)
  {
		if((pic_index_arr[j]-1) == i){
			ws281x_setPixelColor(i, ws281x_color(255,200,0));
			if(j<77)
				j++;
		}else{
			ws281x_setPixelColor(i, ws281x_color(64,0,0));
		}
  }
	ws281x_show();
}
//unisoc old icon
void ws281x_256square2(uint8_t *pic_index_arr)
{
	int16_t i = 0;
	int16_t j = 0;
    for(i = 0; i < PIXEL_NUM; ++i)
  {
		if((pic_index_arr[j]-1) == i){
			ws281x_setPixelColor(i, ws281x_color(128,0,128));//zise
			if(j<114)
				j++;
		}else{
			ws281x_setPixelColor(i, ws281x_color(0,0,0));//back
		}
  }
	ws281x_show();
}
//unisoc new icon
void ws281x_256square3(uint8_t *pic_index_arr)
{
	int16_t i = 0;
	int16_t j = 0;
    for(i = 0; i < PIXEL_NUM; ++i)
  {
		if((pic_index_arr[j]-1) == i){
			ws281x_setPixelColor(i, ws281x_color(128,0,110));//icon color
			if(j<80)
				j++;
		}else{
			ws281x_setPixelColor(i, ws281x_color(0,0,0));//bgcolor black
		}
  }
	ws281x_show();
}

void ws281x_256square_publish(uint8_t index, uint8_t *pic_index_arr, uint32_t *fgcolor_arr, uint32_t *bgcolor_arr)
{
	uint8_t icon_led_count = pic_index_arr[0];
	
	int16_t i = 0;
	int16_t j = 1;//j=0 is count, from j=1 is data
	
  for(i = 0; i < PIXEL_NUM; ++i)
  {
		if((pic_index_arr[j]-1) == i){
			ws281x_setPixelColor(i, fgcolor_arr[index]);//icon color
			if(j<icon_led_count)
				j++;
		}else{
			ws281x_setPixelColor(i, bgcolor_arr[index]);//bgcolor black
		}
  }
	ws281x_show();
