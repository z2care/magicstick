...
void ws281xs_300redcage()
{
  ...
  //rainbow then chase then flow
	int16_t i = 0;
	int16_t j = 0;
    for(i = 0; i < PIXEL_NUM2; ++i)
  {
		ws281xs_setPixelColor(i, ws281xs_color(0,64,64));
  }
	ws281xs_show();
}
...
void ws281xs_256square(uint8_t *pic_index_arr)
{
    ...
    for(i = 0; i < PIXEL_NUM2; ++i)
  {
		if((pic_index_arr[j]-1) == i){
			ws281xs_setPixelColor(i, ws281xs_color(0,128,0));
			if(j<77)
				j++;
		}else{
			ws281xs_setPixelColor(i, ws281xs_color(128,0,0));
		}
  }
	ws281xs_show();
}
